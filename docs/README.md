...
The following notation is used to isolate vectors from the matrix **M**: $$m*{,j}$$ represents the *j*th column vector and $$m*{i,}$$ represents the *i*th row vector in column vector form
...
- **GL_ARB_shader_draw_parameters**: Also introduced in OpenGL 4.6, adds new built-in variables to GLSL vertex shader inputs, _gl_BaseVertexARB_, _gl_BaseInstanceARB_ and _gl_DrawID_
